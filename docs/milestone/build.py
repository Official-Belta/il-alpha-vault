...
<style>
  ...
  * {{ margin:0; padding:0; box-sizing:border-box; }}
  ...
  .hero-back {{ font-family:var(--sans); font-size:20px; font-weight:700; color:var(--white); text-decoration:none; display:inline-flex; align-items:center; gap:12px; margin-bottom:40px; padding:18px 36px; background:var(--accent); border:none; border-radius:8px; transition:all 200ms var(--ease); letter-spacing:-0.01em; }}
  .hero-back:hover {{ background:var(--gray-700); color:var(--white); transform:translateX(-4px); }}
  ...
  .detail-cat {{ display:inline-block; font-family:var(--sans); font-size:16px; font-weight:700; text-transform:uppercase; letter-spacing:0.06em; color:var(--white); background:var(--accent); padding:8px 20px; border-radius:100px; margin-bottom:24px; }}
  .detail-cat[data-cat="dev"] {{ background:#1A7F64; }}
  .detail-cat[data-cat="strategy"] {{ background:#4A6CF7; }}
  .detail-cat[data-cat="design"] {{ background:#E06B5E; }}
...
<section class="hero">
  <a class="hero-back" href="../index.html">&larr; <span data-ko="메인으로 돌아가기">Back to home</span></a>
...
<style>
  ...
  * {{ margin: 0; padding: 0; box-sizing: border-box; }}
  ...
  .log-cat {{ font-family: var(--sans); font-size: 14px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.06em; color: var(--white); background: var(--accent); display: inline-block; padding: 5px 14px; border-radius: 100px; margin-bottom: 14px; }}
  .log-cat[data-cat="dev"] {{ background:#1A7F64; }}
  .log-cat[data-cat="strategy"] {{ background:#4A6CF7; }}
  .log-cat[data-cat="design"] {{ background:#E06B5E; }}
