...
<style>
  ...
  * {{ margin: 0; padding: 0; box-sizing: border-box; }}
  ...
  .logo {{ font-family: var(--sans); font-weight: 700; font-size: 18px; letter-spacing: -0.03em; color: var(--black); text-decoration: none; }}
  .fab-vault {{ position: fixed; bottom: 0; right: 48px; z-index: 90; text-decoration: none; color: var(--white); background: var(--black); padding: 20px 32px; display: flex; flex-direction: column; gap: 8px; transform: translateY(calc(100% - 56px)); transition: transform 400ms var(--ease), box-shadow 400ms var(--ease); }}
  .fab-vault:hover {{ transform: translateY(0); box-shadow: 0 -8px 40px rgba(0,0,0,0.2); }}
  .fab-vault .fab-label {{ font-family: var(--sans); font-size: 16px; font-weight: 600; letter-spacing: -0.01em; }}
  .fab-vault .fab-desc {{ font-family: var(--mono); font-size: 13px; color: var(--gray-500); line-height: 1.5; max-width: 280px; }}
  @media (max-width: 640px) {{ .fab-vault {{ right: 0; left: 0; padding: 16px 24px; }} }}
  .nav-links {{ display: flex; align-items: center; gap: 28px; }}
  .nav-links a {{ font-size: 15px; font-weight: 500; color: var(--gray-600); text-decoration: none; transition: color 300ms var(--ease); }}
  .nav-links a:hover {{ color: var(--black); }}
...
<a class="fab-vault" href="vault.html">
  ...
  <span class="fab-desc" data-ko="변동성이 높을 때 LP를 자동 제거. Sharpe 3.66. 성과 수수료만.">Removes LP when volatility spikes. Sharpe 3.66. Performance-fee-only.</span>
</a>
