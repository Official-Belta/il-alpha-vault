...
<style>
  ...
  * {{ margin: 0; padding: 0; box-sizing: border-box; }}
  ...
  .logo {{ font-family: var(--sans); font-weight: 700; font-size: 18px; letter-spacing: -0.03em; color: var(--black); text-decoration: none; }}
  .fab-vault {{ position: fixed; bottom: 0; right: 48px; z-index: 90; text-decoration: none; color: var(--white); background: rgba(12,12,12,0.92); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); width: 340px; padding: 28px 32px; display: flex; flex-direction: column; gap: 16px; transform: translateY(calc(100% - 56px)); transition: transform 400ms var(--ease); opacity: 0; pointer-events: none; }}
  .fab-vault.visible {{ opacity: 1; pointer-events: auto; }}
...
<script>
  ...
  if (fabVault) {{
    window.addEventListener('scroll', () => {{
      fabVault.classList.toggle('visible', window.scrollY > window.innerHeight * 0.6);
    }});
  }}
